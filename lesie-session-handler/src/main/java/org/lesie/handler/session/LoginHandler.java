...
public class LoginHandler implements LesieHandler {
    ...
    public LesieResponseDTO processRequest(LesieRequestDTO requestDTO) {
        ...
        if(sessionStorage.isRegisteredApp(appIdentityKey)){
            String sessionKey = sessionStorage.generateSessionKey(appIdentityKey);
            response.setSessionToken(sessionKey);
